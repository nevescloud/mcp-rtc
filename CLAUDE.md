...
2. **Add folder share to `/h/`** (File System Access `showDirectoryPicker`). One bind, two tools (`list_files`, `read_file_at`) — the existing capability spec assumes one tool per binding, so this needs a small spec extension (list of tools per cap or a "registers" callback that emits multiple). `read_shared_file` (single file via `showOpenFilePicker`) is **done**.
