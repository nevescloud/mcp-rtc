...
1. **Add screen-share to `/h/`** as the first user-bound capability. The flow: page detects `getDisplayMedia` → renders a "Share screen" button (gesture-bound) → user clicks → media stream opens → `capture_screen()` tool registers; each call uses `ImageCapture.grabFrame()` and returns a base64 PNG. Asker side: Claude Code via the existing stdio bridge (Path B) — known to handle image content in tool responses. **Validate Path B end-to-end before promising the Claude.ai variant** (Anthropic ext handling binary image payloads from a peer MCP server is currently untested).
2. **Add file + folder share** to `/h/` (File System Access API). `read_shared_file` for a user-picked file; `list_files` + `read_file_at(path)` for a user-picked directory. Chrome-only as of 2026-05. Same gesture-bound pattern as screen-share.
3. ~~Add auto-bound device capabilities~~ — **done** (`take_photo`, `get_location`, `read_clipboard`, `get_orientation` live in `/h/`).
